...
## 9 融合推进现状更新（2026-04-16）

在最初版本的基线判断里，仓库的主实现仍然集中在“毫米波雷达采集、传输、清洗和 BP 成像”，而“视觉接入 / 雷达视觉融合”更多停留在项目目标层。到 2026-04-16，这个判断需要更新得更精确一些：

### 9.1 已经新增的真实基础

当前工作区已经补上两块可以直接落地的基础设施：

1. 视觉侧已经在 `vision_uav/` 下形成了一条独立的正式基线分支。
   - 已完成 `Anti-UAV300` 数据准备、A800 正式训练、测试评估、三段视频离线推理、ONNX 导出和本地备份。
   - 当前正式模型为 `vision_uav/runs/train/anti_uav_rgb_yolo26s_a800_b643/weights/best.pt`。
   - 当前推荐离线推理阈值为 `0.45`。
2. `mmradar` 侧已经新增离线融合所需的接口层。
   - `config/sar_tcp.ini` 新增 `[fusion_export]` 配置段。
   - `sar_bp_1d` 已经可以按会话导出 `radar_frames.jsonl`、`session_meta.json`、逐帧 BP 快照和 `final_res.jpg`。
   - 仓库内新增了 `mmradar/tools/fusion/`，可做演示雷达会话生成、时间对齐、决策级融合和并排视频输出。

### 9.2 这不等于“融合已经完成”

虽然接口和演示链已经具备，但当前仍然不能把仓库状态描述成“雷达-视觉融合系统已经完成”。原因很具体：

- 还没有真实的雷达+视频同步采样会话。
- 还没有相机和雷达之间的外参标定。
- 还没有目标框级或坐标级空间融合。
- 当前融合脚本使用的是**离线最近邻时间对齐 + 决策级状态判定**。
- 当前本地会话 `mmradar/resources/fusion_sessions/demo_20190925_111757_1_10/` 是接口演示样例，不是性能评估样本。

### 9.3 更准确的当前状态描述

所以，到这一步更准确的判断应当是：

> 项目已经从“只有雷达链路”推进到“视觉正式基线已独立落地、雷达侧融合导出接口已补齐、离线融合 MVP 已经跑通”，但仍未进入“真实同步样本上的融合性能验证”阶段。

### 9.4 对后续接手的直接建议

如果下一步继续推进融合，不要跳过中间层，建议顺序固定为：

1. 先积累真实的雷达+视频同步样本。
2. 保持当前 `predictions.jsonl` 与 `radar_frames.jsonl` 契约不变。
3. 在真实样本上先验证时间对齐，再做标定和空间级融合。
4. 不要把当前 demo 会话误当作算法性能结论。
